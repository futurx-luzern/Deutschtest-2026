...
create or replace function admin_unlock_assignment(p_session_token text, p_assignment_id uuid)
returns void
language plpgsql security definer as $$
begin
  perform assert_admin(p_session_token);
...
create or replace function admin_list_assignments_overview(p_session_token text)
returns table(
  assignment_id uuid,
  participant_id uuid,
  pseudonym_id text,
...
begin
  ...
  select
    ...
    a.progress_percent,
    a.last_activity_at,
    coalesce((a.state->>'totalActiveTimeMs')::bigint, 0) as total_active_time_ms
  from test_assignments a
  join participants p on p.id = a.participant_id
  where p.active = true
  order by p.display_name nulls last, p.pseudonym_id, a.test_key;
end;
$$;

create or replace function admin_export_assignment_json(p_session_token text, p_assignment_id uuid)
returns jsonb
language plpgsql security definer as $$
declare
  v_participant participants;
  v_assignment test_assignments;
begin
  perform assert_admin(p_session_token);
  ...
  select * into v_participant from participants where id = v_assignment.participant_id;
  if v_participant.id is null then
    return jsonb_build_object('ok', false, 'message', 'Teilnehmende nicht gefunden');
  end if;
  ...
  return jsonb_build_object(
    ...
    'payload', jsonb_build_object(
      'participant', jsonb_build_object(
        'id', v_participant.id,
        'pseudonym_id', v_participant.pseudonym_id,
        'display_name', v_participant.display_name
      ),
      'assignment', jsonb_build_object(
        'assignment_id', v_assignment.id,
        'test_key', v_assignment.test_key,
        'status', v_assignment.status,
        'locked', v_assignment.locked,
        'progress_percent', v_assignment.progress_percent,
        'state', v_assignment.state,
        'last_activity_at', v_assignment.last_activity_at,
...
grant execute on function admin_unlock_assignment(text,uuid) to anon, authenticated;
grant execute on function admin_list_assignments_overview(text) to anon, authenticated;
grant execute on function admin_export_assignment_json(text,uuid) to anon, authenticated;
